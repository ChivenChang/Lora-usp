...
```bash
rm -Rf build/ ; cmake -L -S examples/main_examples/flrc_burst_example -B build -DCMAKE_BUILD_TYPE=Debug -DBOARD=NUCLEO_L476 -DRAC_RADIO=lr2021 -DRP_MARGIN_DELAY=10 -G Ninja; cmake --build build --target flrc_burst_tx flrc_burst_rx
```

**Non Public Legacy LR20XX Shield:**
```bash
rm -Rf build/ ; cmake -L -S examples/main_examples/flrc_burst_example -B build -DCMAKE_BUILD_TYPE=Debug -DBOARD=NUCLEO_L476 -DRAC_RADIO=lr2021 -DLEGACY_EVK_LR20XX=ON -DRP_MARGIN_DELAY=10 -G Ninja; cmake --build build --target flrc_burst_tx flrc_burst_rx
```
