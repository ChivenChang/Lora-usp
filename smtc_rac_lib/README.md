...
- `pre-release`: pre-release version (char*) string that begins with hyphen and must comprise only ASCII alphanumerics and hyphens [0-9A-Za-z-]
- `metada`: metada version (char*) string that begins with a plus sign must comprise only ASCII alphanumerics and hyphens [0-9A-Za-z-]
